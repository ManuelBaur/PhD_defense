\frame{
	...
	\begin{textblock}{0.9}(0.05,0.35)
		...
		{\Huge
			\textcolor{white}{
				Validating X-DFA on a sedimenting suspension}}\\[0.2cm]
		{\huge
			\textcolor{white}{
				Measurements on a fluidized bed}}
	\end{textblock}
}

\frame{
\frametitle{Experimental validation: Sedimenting suspension}

\begin{textblock}{0.9}(0.05,0.1)	
track sedimentation front $\leftrightarrow$ DFA
\end{textblock}

\begin{textblock}{0.9}(0.05,0.2)
\centering
\only<1>{
\includegraphics[width=\textwidth]{Sources/sedimenting_bed/setup-photo_and_fluidized_bed_sketch.pdf}}	
\end{textblock}	
...
{Sources/sedimenting_bed/ROI_marked_img_nr1.png}}
\end{textblock}

\begin{textblock}{0.9}(0.05,0.85)
\centering	
\only<2>{
\href{run:Sources/sedimenting_bed/tracked_edge.avi}{\textcolor{red}{video of particle front}}}
\end{textblock}
}
...
\frame{
	...
	\begin{textblock}{0.6}(0.0,0.08)	
		\begin{align}
		I(\mathbf{r},t) = I_0 + \int \mathrm{d}\mathbf{r}' \ \mathrm{d}z'

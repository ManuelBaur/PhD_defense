...
file_fitParam = (dir_out + 'fit_parameter_values_for_f_q_tau_diffusive.dat')
...
### perfrom fit
	...
	def fit_func(tau,a0,a2):
		return np.log(a0 * (1-np.exp(-tau/a2)))
	...
	for nn_q in np.arange(N_q): # [132]: # 
		...
		popt, pcov = curve_fit(fit_func, tau, np.log(d_q),\
		 	p0 = [Params0[0], Params0[1]],\
			bounds = param_bounds)

		A_vec[nn_q] = popt[0]
		B_vec[nn_q] = 0. # popt[1]
		tau_d_vec[nn_q] = popt[1] # diffusion time for all q-values	

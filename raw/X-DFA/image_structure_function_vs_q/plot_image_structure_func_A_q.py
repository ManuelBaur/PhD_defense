...
########## read A(q)
nam_AB = 'fit_parameter_values_for_f_q_tau_diffusive.dat'
data_AB = np.loadtxt(nam_AB)
A_q = data_AB[:,1]
...
########## add A(q) to plot
col_A = 'C1'
ax.plot(vec_q[:,0], A_q, color = col_A, linewidth='2.5') 
plt.annotate('$A(q) + B(q)$', xy=(0.55,0.8), xycoords='axes fraction', color=col_A)


fig.savefig('img_struc_func_vs_q_multiple_tau_simulations_nPart'
	+ str(format(nPart)) + 'with_A.eps',
